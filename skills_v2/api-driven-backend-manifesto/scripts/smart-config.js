...
async function runConfigWizard() {
    log('\n🚀 Smart Configuration Manager', 'cyan');
    log('━'.repeat(50), 'cyan');

    // Detect environment
    log('\n📋 Detecting environment...', 'blue');
    const hasMysql = detectMySQL();
    const hasRedis = detectRedis();

    log(`  MySQL: ${hasMysql ? '✅ Installed' : '❌ Not found'}`, hasMysql ? 'green' : 'yellow');
    log(`  Redis: ${hasRedis ? '✅ Installed' : '❌ Not found'}`, hasRedis ? 'green' : 'yellow');

    if (!hasMysql) {
        log('\n⚠️  MySQL not detected. Please install MySQL first.', 'yellow');
    ...
    config.db.port = await question(`  Port [3306]: `) || '3306';
    config.db.user = await question(`  User [root]: `) || 'root';
    config.db.password = await question(`  Password: `);
    config.db.database = await question(`  Database name [enterprise_db]: `) || 'enterprise_db';

    // Test database connection
    log('\n🔍 Testing database connection...', 'blue');
    const dbConnected = testMySQLConnection(
        config.db.host,
        config.db.port,
        config.db.user,
        config.db.password
    ...
    if (hasRedis) {
        ...
        log('  ⚠️  Redis not detected, using defaults', 'yellow');
    ...
    log('\n📝 Generating configuration file...', 'blue');
